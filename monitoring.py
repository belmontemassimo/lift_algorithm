...
class Monitoring:
    ...
    # class that handles the monitoring GUI for lift simulation
    # displays lift positions, speeds, states, weights, and target floors
    # allows setting simulation parameters and starting the simulation
    ...
    def __init__(self, queue: Queue, algorithms: list[str], capacity: int):
        # initialize monitoring interface with queue for communication
        # samples list for selection
        # algorithms list for selection
        self.queue = queue
        ...
        # create labels for displaying simulation state
        ...
        # create dropdown selections for algorithm and sample
    ...
    def update(self, _):
        # update the display with the latest simulation data
        # gets data from queue and updates all UI elements
        if self.queue.empty():
            return
    ...
    def on_start_update(self, _):
        ...
        # gets user inputs and sends configuration to simulation
        # disables controls after simulation starts
        try:
            self.queue.put({
            ...
            # disable all controls once simulation starts
        ...
        # wait until queue is empty before starting timer
        while True:
            if self.queue.empty():
                break
        self.timer.Start(40)
...
class Monitoring:
    ...
    def __init__(self, queue: Queue, algorithms: list[str], capacity: int):
        # initialize monitoring interface with queue for communication
        # algorithms list for selection and capacity constant
        self.queue = queue
        self.capacity = capacity
        self.samples = samples_list()
        self.algorithms = algorithms
...
def run_monitoring(algorithms: list[str], capacity: int) -> Queue:
    # function to start monitoring in a separate process
    # returns a queue for communication with the main simulation
    queue = Queue()
...
def update_monitoring(queue: Queue, lift_manager: LiftManager, timer: float):
    # function to update monitoring with current simulation state
    # collects and formats data from lift_manager and sends it via queue
    if queue.empty():
